...
switch ($_GET['jenis']) {
    ...
    case 'kota':
        ...
        if ($id_provinces == '') {
            ...
            $getcity = $db->tampildata3("regencies", "and province_id ='$id_provinces'");
            while ($data = $getcity->fetch_array()) {
    ...
    case 'kecamatan':
        ...
        if ($id_regencies == '') {
            ...
            $getcity = $db->tampildata3("districts", "and regency_id ='$id_regencies'");
            while ($data = $getcity->fetch_array()) {
                echo '<option value="' . $data['id'] . '">' . $data['name'] . '</option>';
            }
            exit;
